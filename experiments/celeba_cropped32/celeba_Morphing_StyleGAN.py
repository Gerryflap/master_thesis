...
parser = argparse.ArgumentParser(description="Celeba Morphing StyleGAN experiment.")
...
output_path = util.output.init_experiment_output_dir("celeba32", "Morphing_StyleGAN", args)
...
Gx = StyleGenerator(args.l_size, args.h_size, 4, 3, mapping_depth=8)
D = DeepAliDiscriminator(args.l_size, args.h_size, 32, 3, bn=False)
G_syn_optimizer = torch.optim.Adam(list(Gx.get_synthesis_parameters()) + list(Gz.parameters()), lr=args.lr, betas=(0, 0.9))
G_map_optimizer = torch.optim.Adam(Gx.get_mapping_parameters(), lr=args.lr/100.0, betas=(0, 0.9))
