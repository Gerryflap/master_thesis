...
class EncoderMorphNet28(Encoder28):
    def __init__(self, latent_size, h_size, use_mish=False, n_channels=1, deterministic=False, cap_variance=True,
                 add_dense_layer=False, use_lr_norm=False, block_Gz_morph_grads=False):
        super().__init__(latent_size, h_size, use_mish, n_channels, deterministic, cap_variance, add_dense_layer,
                         use_lr_norm)
    ...
    def pretrain_morph_network(self, n_batches=10000, batch_size=64):
        ...
        pretrain_opt = torch.optim.Adam(self.morph_net.parameters(), 0.001)
