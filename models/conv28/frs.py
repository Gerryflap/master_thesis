...
class FRS28(torch.nn.Module):
    def __init__(self, latent_size, h_size, use_mish=False, n_channels=1, add_dense_layer=False, hypersphere_output=True, batchnorm=False):
        super().__init__()
        ...
        self.h_size = h_size
        self.conv_1 = torch.nn.Conv2d(n_channels, h_size, kernel_size=4, stride=1, bias=not batchnorm)
        self.conv_2 = torch.nn.Conv2d(h_size, h_size * 2, kernel_size=5, stride=2, bias=not batchnorm)
        self.conv_3 = torch.nn.Conv2d(h_size * 2, h_size * 4, kernel_size=5, stride=2, bias=not batchnorm)
        self.conv_4 = torch.nn.Conv2d(h_size * 4, h_size * 4, kernel_size=4, stride=1, bias=not batchnorm)
        ...
        if add_dense_layer:
            self.dense = torch.nn.Linear(h_size * 4, h_size * 4, bias=not batchnorm)
            if self.bn:
                self.bn_dense = torch.nn.BatchNorm1d(self.h_size * 4)

        self.hypersphere_output = hypersphere_output
