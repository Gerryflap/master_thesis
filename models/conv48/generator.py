...
class Generator48(torch.nn.Module):
    def __init__(self, latent_size, h_size, use_mish=False, n_channels=3, sigmoid_out=False, use_lr_norm=False):
        ...
        self.conv_2 = torch.nn.ConvTranspose2d(self.h_size * 8, self.h_size * 4, kernel_size=7, stride=1, bias=False)
        self.conv_3 = torch.nn.ConvTranspose2d(self.h_size * 4, self.h_size*4, kernel_size=4, stride=2, bias=False)
        self.conv_4 = torch.nn.ConvTranspose2d(self.h_size * 4, self.h_size*2, kernel_size=5, stride=2, bias=False)
        ...
        self.output_bias = torch.nn.Parameter(torch.zeros((3, 48, 48)), requires_grad=True)
...
if __name__ == "__main__":
    G = Generator48(256, 64)
    print(G(torch.normal(0, 1, (1, 256))).size())
